...
function TableReportYear() {
  ...
  const {
    ...
    `${process.env.REACT_APP_API_REPORT}/partner/get-count-allday-reserv-per-year`,
      ...
      <div className="h-[600px]  overflow-x-auto rounded-lg ">
        <div className="inline-block min-w-full shadow-md rounded-lg ">
          <table className="min-w-full table-auto leading-normal">
            ...
            <tbody>
              {countReservPerYear .slice(pagesVisited, pagesVisited + dataPerPage).map((count: Resualt, index: number) => {
                return (
                  <tr key={index}>
