...
const selectYear = years2.concat(
  years1.filter((item) => years2.indexOf(item) < 0)
);
...
function TableReportMonth() {
  ...
  const {
    ...
    `${process.env.REACT_APP_API_REPORT}/partner/get-count-round-reserv-per-month`,
      ...
      <div className="h-[600px]   rounded-lg ">
        <div className="inline-block min-w-full shadow-md rounded-lg ">
          <table className="min-w-full table-auto leading-normal">
            ...
            <tbody>
              {(type === "all" ? sortLabelsYear : dataSets)
                .slice(pagesVisited, pagesVisited + dataPerPage)
                .map((count: Resualt, index: number) => {
                  return (
                    <tr key={index}>
                      <td className="px-1 py-2 border-b border-gray-200 bg-white text-sm text-center">
                        <p className="text-gray-900 whitespace-no-wrap">
                          {`เดือนที่ ${count._id.month}/${count._id.year}
                    `}
                        </p>
                      </td>
                      <td className="px-1 py-2 border-b border-gray-200 bg-white text-sm text-center">
