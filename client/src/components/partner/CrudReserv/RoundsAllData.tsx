...
const RoundsAllData = observer(() => {
  ...
  useEffect(() => {
    const getRoundReservs = async () => {
      await reservStore.getRoundReserv();
    };
    getRoundReservs();
  }, []);
  const roundReservs = reservStore.roundReservs;
  const partner = partnerStore.partnerLogin;
  const pageCount = Math.ceil(roundReservs.length / dataPerPage);
  ...
  const confirmDelete = (id: string) => {
    Swal.fire({
      ...
      if (resualt.isConfirmed) {
        reservStore.deleteRoundReserv(id);
      }
    });
  };
                ...
                <tr  key={index} className="bg-white hover:bg-gray-100">
                  ...
                  <td className="px-2 py-3 border-b border-gray-200  text-sm ">
                    <button
                      ...
                      onClick={() => {
                        navigate(
                          `/partner/reservationdata/round/edit/${reserv.partner_id}/${reserv._id}`
                        );
                      }}
                    >
                      แก้ไข
